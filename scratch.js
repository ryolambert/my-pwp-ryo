...
export default {
  ...
  data() {
    return {
      world: null,
      container: document.getElementsByClassName('world')[0],
  ...
  methods: {
    ...
    mouseMove(mousePos) {
      this.targetMousePos.x = this.mousePos.px;
      this.targetMousePos.y = this.mousePos.py;
    },
    handleWindowResize() {
      this.init(this.updateSize(window.innerWidth, window.innerHeight));
    },
    handleMouseMove(e) {
      ...
      this.init(this.mouseMove(this.mousePos));
    },
    fragmentShader() {
      return `
